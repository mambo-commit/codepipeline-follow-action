...
async function excutionRevisionCheck (credentials: aws_sdk.Credentials, bucketName: string, artifactName: string) {
    let params
    params = {
      apiVersion: 'latest',
      region: 'us-east-1',
...
async function isRunning (credentials: aws_sdk.Credentials) {
    ...
    let stopState = ['Succeeded', 'Stopped', 'Cancelled', 'Superseded', 'Failed', 'noresponse']
    let isRunning = true
    console.log(executionStatus, executionDetails.pipelineExecutionSummaries?.at(0)?.lastUpdateTime)
